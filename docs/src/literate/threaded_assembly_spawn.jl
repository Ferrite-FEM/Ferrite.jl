...
# the FaceValues)
function create_values(refshape, order::Int)
...
function create_scratchbuffer(K, f, dh::DofHandler)
    ...
    cellvalues, facevalues = create_values(RefCube, 2)
...
#function run_assemble()
    const refshape = RefCube
    const quadrature_order = 2
    const dim = 3
    const n = 20
    const grid, colors = create_colored_cantilever_grid(Hexahedron, n);
    const dh = create_dofhandler(grid);

    const K = create_sparsity_pattern(dh);
    const C = create_stiffness();
    ## compilation
