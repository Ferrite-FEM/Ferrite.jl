...
# [`MultiFieldCellValues`](@ref) to demonstrate this alternative. Here, we
...
function assemble_multiplier_terms!(assembler, dh, coupling, fvp, Γ)
    ...
    for facet in FacetIterator(dh, Γ)
        reinit!(fvp, facet)
        layout = local_dofs(facet, coupling)
...
function main_multiplier(h = 0.05)
    ...
    ## Check the result against the AffineConstraint variant #src
    if h == 0.05                        #src
        check_L2(dh, cvu, cvp, a)       #src
        check_mean_constraint(dh, fvp, a) #src
    end                                 #src
