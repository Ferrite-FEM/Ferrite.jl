# Common methods for both FEValues and FEFaceValues
typealias FEValuesOrFEFaceValues{dim,T,FS,GS} Union{FEValues{dim,T,FS,GS}, FEFaceValues{dim,T,FS,GS}}

"""
The quadrature rule for the `FEValues`/`FEFaceValues` type.
...
* `fe_values`: the `FEValues`/`FEFaceValues` object
...
get_quadrule(fe_fv::FEFaceValues) = fe_fv.quad_rule[fe_fv.current_boundary[]]

"""
The function space for the `FEValues`/`FEFaceValues` type.
...
"""
get_functionspace(fe_v::FEValuesOrFEFaceValues) = fe_v.function_space

"""
The function space used for geometric interpolation for the `FEValues`/`FEFaceValues` type.
...
"""
get_geometricspace(fe_v::FEValuesOrFEFaceValues) = fe_v.geometric_space
...
"""
@inline function function_scalar_value{dim, T}(fe_v::FEValuesOrFEFaceValues{dim}, q_point::Int, u::Vector{T})
...
"""
@inline function function_vector_value{dim, T}(fe_v::FEValuesOrFEFaceValues{dim}, q_point::Int, u::Vector{Vec{dim, T}})
    n_base_funcs = n_basefunctions(get_functionspace(fe_v))
    @assert length(u) == n_base_funcs
    vec = zero(Vec{dim, T})
    N = shape_value(fe_v, q_point)
...
"""
@inline function function_scalar_gradient{dim, T}(fe_v::FEValuesOrFEFaceValues{dim}, q_point::Int, u::Vector{T})
    n_base_funcs = n_basefunctions(get_functionspace(fe_v))
    @assert length(u) == n_base_funcs
...
"""
@inline function function_vector_gradient{dim, T}(fe_v::FEValuesOrFEFaceValues{dim}, q_point::Int, u::Vector{Vec{dim, T}})
    n_base_funcs = n_basefunctions(get_functionspace(fe_v))
    @assert length(u) == n_base_funcs
    dN = shape_gradient(fe_v, q_point)
...
where ``\\mathbf{u}_i`` are the nodal values of the function.
"""
@inline function function_vector_symmetric_gradient{dim, T}(fe_v::FEValuesOrFEFaceValues{dim}, q_point::Int, u::Vector{Vec{dim, T}})
...
"""
@inline function function_vector_divergence{dim, T}(fe_v::FEValuesOrFEFaceValues{dim}, q_point::Int, u::Vector{Vec{dim, T}})
    n_base_funcs = n_basefunctions(get_functionspace(fe_v))
    @assert length(u) == n_base_funcs
    dN = shape_gradient(fe_v, q_point)
    diverg = zero(T)
