...
function generate_grid(C::Type{Quadrilateral}, nel::NTuple{2,Int}, LL::Vec{2,T}, LR::Vec{2,T}, UR::Vec{2,T}, UL::Vec{2,T}) where {T}
    ...
    facesets["bottom"] = Set{FaceIndex}(boundary[(1:length(cell_array[:, 1])).+offset])
    offset             += length(cell_array[:, 1])
    facesets["right"]  = Set{FaceIndex}(boundary[(1:length(cell_array[end, :])).+offset])
    offset             += length(cell_array[end, :])
    facesets["top"]    = Set{FaceIndex}(boundary[(1:length(cell_array[:, end])).+offset])
    offset             += length(cell_array[:, end])
    facesets["left"]   = Set{FaceIndex}(boundary[(1:length(cell_array[1, :])).+offset])
    offset             += length(cell_array[1, :])
...
function generate_grid(::Type{QuadraticQuadrilateral}, nel::NTuple{2,Int}, LL::Vec{2,T}, LR::Vec{2,T}, UR::Vec{2,T}, UL::Vec{2,T}) where {T}
    ...
    for j in 1:nel_y, i in 1:nel_x
        push!(
            cells,
            QuadraticQuadrilateral((node_array[2*i-1, 2*j-1], node_array[2*i+1, 2*j-1], node_array[2*i+1, 2*j+1], node_array[2*i-1, 2*j+1],
...
function generate_grid(::Type{Hexahedron}, nel::NTuple{3,Int}, left::Vec{3,T} = Vec{3}((-1.0, -1.0, -1.0)), right::Vec{3,T} = Vec{3}((1.0, 1.0, 1.0))) where {T}
    ...
    for k in 1:nel_z, j in 1:nel_y, i in 1:nel_x
        push!(
            cells,
            Hexahedron((node_array[i, j, k], node_array[i+1, j, k], node_array[i+1, j+1, k], node_array[i, j+1, k],
    ...
    facesets["bottom"] = Set{FaceIndex}(boundary[(1:length(cell_array[:, :, 1][:])).+offset])
    offset             += length(cell_array[:, :, 1][:])
    facesets["front"]  = Set{FaceIndex}(boundary[(1:length(cell_array[:, 1, :][:])).+offset])
    offset             += length(cell_array[:, 1, :][:])
    facesets["right"]  = Set{FaceIndex}(boundary[(1:length(cell_array[end, :, :][:])).+offset])
    offset             += length(cell_array[end, :, :][:])
    facesets["back"]   = Set{FaceIndex}(boundary[(1:length(cell_array[:, end, :][:])).+offset])
    offset             += length(cell_array[:, end, :][:])
    facesets["left"]   = Set{FaceIndex}(boundary[(1:length(cell_array[1, :, :][:])).+offset])
    offset             += length(cell_array[1, :, :][:])
    facesets["top"]    = Set{FaceIndex}(boundary[(1:length(cell_array[:, :, end][:])).+offset])
...
function Ferrite.generate_grid(::Type{Cell{3,20,6}}, nel::NTuple{3,Int}, left::Vec{3,T} = Vec{3}((-1.0, -1.0, -1.0)), right::Vec{3,T} = Vec{3}((1.0, 1.0, 1.0))) where {T}
    ...
    nnode_x = 2nel_x + 1
    nnode_y = 2nel_y + 1
    nnode_z = 2nel_z + 1 #Note: not the actually number of nodes in x/y/z, just a temporary variables
...
function generate_grid(::Type{Triangle}, nel::NTuple{2,Int}, LL::Vec{2,T}, LR::Vec{2,T}, UR::Vec{2,T}, UL::Vec{2,T}) where {T}
    ...
    offset             = 0
    facesets           = Dict{String,Set{FaceIndex}}()
    facesets["bottom"] = Set{FaceIndex}(boundary[(1:length(cell_array[1, :, 1])).+offset])
    offset             += length(cell_array[1, :, 1])
    facesets["right"]  = Set{FaceIndex}(boundary[(1:length(cell_array[2, end, :])).+offset])
    offset             += length(cell_array[2, end, :])
    facesets["top"]    = Set{FaceIndex}(boundary[(1:length(cell_array[2, :, end])).+offset])
    offset             += length(cell_array[2, :, end])
    facesets["left"]   = Set{FaceIndex}(boundary[(1:length(cell_array[1, 1, :])).+offset])
    offset             += length(cell_array[1, 1, :])
