...
function generate_simple_disc_grid(::Type{Quadrilateral}, n; radius = 1.0)
    ...
    facesets = Dict(
        "boundary" => Set([FaceIndex(i, 1) for i in 1:n]) ∪ Set([FaceIndex(i, 2) for i in 1:n]),
    )
...
function generate_simple_disc_grid(::Type{Hexahedron}, n; radius = 1.0, layers = 1, height = 1.0)
    ...
    facesets = Dict(
        "side" => Set([FaceIndex(i, 1) for i in 1:n]) ∪ Set([FaceIndex(i, 2) for i in 1:n]),
        "top" => Set([FaceIndex(i, 5) for i in 1:n]),
        "bottom" => Set([FaceIndex(i, 6) for i in 1:n]),
    )
