...
"""
    CellDofValues(func_interpol::Interpolation, geom_interpol::Interpolation)

`CellDofValues` stores the shape values for the geometric interpolation (`geom_interpol`),
at each "integration point", given by the dof-positions of the function interpolation (`func_interpol`).
Used mainly in the [`apply_analytical!`](@ref) function.
"""
struct CellDofValues{T}
    M::Matrix{T}
end
CellDofValues(func_interpol::Interpolation, args...) = CellDofValues(Float64, func_interpol, args...)

function CellDofValues(::Type{T}, func_interpol::Interpolation, geom_interpol::Interpolation) where {T}
    M = zeros(T, getnbasefunctions(geom_interpol), getnbasefunctions(func_interpol))
    ref_coords = reference_coordinates(func_interpol)
    for (i_dof, ref_coord) in enumerate(ref_coords)
        for i_nod in axes(M, 1)
            M[i_nod, i_dof] = value(geom_interpol, i_nod, ref_coord)
        end
    end
    return CellDofValues(M)
end

getnquadpoints(cdv::CellDofValues) = size(cdv.M, 2) # How many dof locations 
getngeobasefunctions(cdv::CellDofValues) = size(cdv.M, 1)

function spatial_coordinate(cdv::CellDofValues, q_point::Int, xh::AbstractVector{<:Vec})
    n_geom_base_funcs = getngeobasefunctions(cdv)
    if q_point > getnquadpoints(cdv)
        throw(ArgumentError("q_point is higher than number of dof locations for the given function interpolation"))
    end
    length(xh) == n_geom_base_funcs || throw_incompatible_coord_length(length(xh), n_geom_base_funcs)
    x = zero(eltype(xh))
    for i_nod in 1:n_geom_base_funcs
        # value(ip_geo, i_nod, refcoords[q_point]) * xh[i_nod]
        x += cdv.M[i_nod, q_point] * xh[i_nod]
    end
    return x
end