...
function reference_shape_value(ip::Lagrange{RefTetrahedron, 3}, ξ::Vec{3, T}, i::Int) where {T}
    ...
    i == 1 && return T(1.0) + T(-5.5) * ξ_x + T(9.0) * ξ_x^2 + T(-4.5) * ξ_x^3 + T(-5.5) * ξ_y + T(15.75) * ξ_x * ξ_y + T(-13.500000000000005) * ξ_x^2 * ξ_y + T(8.999999999999998) * ξ_y^2 + T(-6.75) * ξ_x * ξ_y^2 + T(-4.5) * ξ_y^3 + T(-5.500000000000001) * ξ_z + T(18.0) * ξ_x * ξ_z + T(-13.5) * ξ_x^2 * ξ_z + T(18.0) * ξ_y * ξ_z + T(-27.0) * ξ_x * ξ_y * ξ_z + T(-13.5) * ξ_y^2 * ξ_z + T(9.0) * ξ_z^2 + T(-13.5) * ξ_x * ξ_z^2 + T(-13.5) * ξ_y * ξ_z^2 + T(-4.5) * ξ_z^3
    ...
    i == 12 && return T(-4.499999999999999) * ξ_z + T(4.499999999999998) * ξ_x * ξ_z + T(4.499999999999998) * ξ_y * ξ_z + T(18.0) * ξ_z^2 + T(-13.5) * ξ_x * ξ_z^2 + T(-13.5) * ξ_y * ξ_z^2 + T(-13.5) * ξ_z^3
