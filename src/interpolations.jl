...
"""
    requires_basis_transformation(::Interpolation)

Return `true` if the interpolation requires a basis transformation when
evaluating shape functions on a physical cell.
"""
requires_basis_transformation(::Interpolation) = false

"""
    reinit_needs_cell(::Interpolation)

Return `true` if `reinit!(cv, cell, coords)` requires the current `cell::AbstractCell` to evaluate or transform
the interpolation's shape functions.
"""
function reinit_needs_cell(ip::Interpolation)
    if mapping_type(ip) isa CovariantPiolaMapping ||
            mapping_type(ip) isa ContravariantPiolaMapping
        return true
    end
    #TODO: also use functionals to determine if reinit_needs_cell
    return false
end
...
"""
@inline function get_direction(::Interpolation, shape_nr, cell)
    return 1
end
...
requires_basis_transformation(::Argyris{RefTriangle, 5}) = true
reinit_needs_cell(ip::Argyris{RefTriangle, 5}) = true
