...
module DofOrder
    ...
    struct CellWise #= <: DofOrdering =# end
...
`order` can be given by one of the following options:
 ...
 - [`DofOrder.CellWise()`](@ref) for renumbering cell wise, ordered by field per cell.
...
function compute_renumber_permutation(dh::AbstractDofHandler, _, perm::AbstractVector{<:Int})
...
Dof ordering passed to [`renumber!`](@ref) to renumber global dofs by cell number. Dofs
within each cell are ordered by field.
"""
DofOrder.CellWise

function compute_renumber_permutation(dh::DofHandler, _, ::DofOrder.CellWise)
    perm = zeros(Int, ndofs(dh))
    seen = sizehint!(Set{Int}(), ndofs(dh))
    n = 0
    flags = UpdateFlags(nodes=false, coords=false, celldofs=true)
    @inbounds for cell in CellIterator(dh, #=cellset=# nothing, flags)
        cdofs = celldofs(cell)
        # sort!(cdofs) # TODO: To make it stable, but is that useful?
        for dof in cdofs; if !(dof in seen)
            perm[dof] = n += 1
            push!(seen, dof)
        end end
    end
    @assert n == length(seen) == ndofs(dh)
    return perm
end
