...
more details):
 - [`create_sparsity_pattern!`](@ref): convenience method for calling
...
function SparsityPattern(nrows::Int, ncols::Int; nnz_per_row::Int = 8)
    ...
    sp = SparsityPattern(nrows, ncols, heap, rows)
    return sp
end
...
function create_sparsity_pattern!(
    ...
    # interface_coupling, topology)
end


# # Arguments
#  - `sp`: the sparsity pattern to add entries to
...
#    internally). See [`add_constraint_entries!`](@ref).
# # Keyword arguments
...
function add_interface_entries!(
        ...
        topology::ExclusiveTopology, keep_constrained::Bool, interface_coupling::AbstractMatrix{Bool},
    )
    if !keep_constrained
        ch === nothing && error("must pass ConstraintHandler when `keep_constrained = true`")
        isclosed(ch) || error("the ConstraintHandler must be closed")
...
function add_diagonal_entries!(sp::AbstractSparsityPattern)
    for d in 1:min(n_rows(sp), n_cols(sp))
        add_entry!(sp, d, d)
    end
    return sp
end
...
(@ref allocate_matrix(::Type{S}, sp::AbstractSparsityPattern) where {Tv, Ti, S <: SparseMatrixCSC{Tv, Ti}}).
"""
...
create_sparsity_pattern!(sp, dh, args...; kwargs...)
