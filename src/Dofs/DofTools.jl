...
Return the (unordered) set of global degrees of freedom matching the filtering `kwargs...`.
...
function collect_dofs!(
    ...
    if cellset !== nothing && faceset !== nothing
        throw(ArgumentError("can not specify both cellset and faceset"))
    ...
    for sdh in dh.subdofhandlers
        ...
        if field !== nothing
            field_idx = findfirst(x -> x === field, sdh.field_names)
