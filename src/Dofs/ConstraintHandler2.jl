...
struct Dirichlet2 # <: Constraint
    f::Function # f(x,t) -> value(s)
    faces::Set{Tuple{Int,Int}}
    field_name::Symbol
    components::Vector{Int} # components of the field
    x::Vector # store the coordinates
    prescribed_dofs::Vector{Int}
    # local_face_dofs::Vector{Int}
    # local_face_dofs_offset::Vector{Int}
end
function Dirichlet2(field_name::Symbol, faces::Union{Set{Int},Set{Tuple{Int,Int}}}, f::Function, component::Int=1)
    Dirichlet2(field_name, faces, f, [component])
end
function Dirichlet2(field_name::Symbol, faces::Union{Set{Int},Set{Tuple{Int,Int}}}, f::Function, components::AbstractVector{Int})
    ...
    return Dirichlet2(f, faces, field_name, Vector(components), Vec{2,Float64}[], Int[])
end
...
"""
struct ConstraintHandler2{DH<:DofHandler,T}
    dbcs::Vector{Dirichlet2}
    prescribed_dofs::Vector{Int}
...
export Dirichlet2, ConstraintHandler2

function ConstraintHandler2(dh::DofHandler)
    @assert isclosed(dh)
    ConstraintHandler2(Dirichlet2[], Int[], Int[], Float64[], Dict{Int,Int}(), dh, ScalarWrapper(false))
end

# function Base.show(io::IO, ::MIME"text/plain", ch::ConstraintHandler2)
#     println(io, "ConstraintHandler2:")
#     if !isclosed(ch)
#         print(io, "  Not closed!")
#     else
#         print(io, "  BCs:")
...
isclosed(ch::ConstraintHandler2) = ch.closed[]
free_dofs(ch::ConstraintHandler2) = ch.free_dofs
prescribed_dofs(ch::ConstraintHandler2) = ch.prescribed_dofs

"""
    close!(ch::ConstraintHandler2)

Close and finalize the `ConstraintHandler2`.
"""
function close!(ch::ConstraintHandler2)
    error()
...
function dbc_check(ch::ConstraintHandler2, dbc::Dirichlet2)
    # check input
    ...
    if length(dbc.faces) == 0
        @warn("added Dirichlet2 Boundary Condition to set containing 0 entities")
    end
end

"""
    add!(ch::ConstraintHandler2, dbc::Dirichlet2)

Add a `Dirichlet2 boundary` condition to the `ConstraintHandler2`.
"""
function add!(ch::ConstraintHandler2, dbc::Dirichlet2)
    dbc_check(ch, dbc)
...
function _add!(ch::ConstraintHandler2, dbc::Dirichlet2, bcfaces::Set{Tuple{Int,Int}}, interpolation::Interpolation, field_dim::Int, offset::Int)
    dh = ch.dh
    grid = dh.grid
    xh = zeros(Vec{2, Float64}, 3) # pre-allocate
    ...
    # loop over all the faces in the set and add the global dofs to `prescribed_dofs`
    prescribed_dofs = Int[]
    ...
    for (cellidx, faceidx) in bcfaces
        ...
        append!(prescribed_dofs, _celldofs[local_face_dofs[r]]) # TODO: for-loop over r and simply push! to ch.prescribed_dofs
        @debug println("adding dofs $(_celldofs[local_face_dofs[r]]) to dbc")
    end

    # save it to the ConstraintHandler2
    append!(dbc.prescribed_dofs, prescribed_dofs)
    push!(ch.dbcs, dbc)
    append!(ch.prescribed_dofs, prescribed_dofs) # TODO: do this in close
end

function _add!(ch::ConstraintHandler2, dbc::Dirichlet2, bcnodes::Set{Int}, interpolation::Interpolation, field_dim::Int, offset::Int)
    ...
    for node in bcnodes
        if !visited[node]
            error("Unable to add a Dirichlet2 boundary condition to node $node as there are no degrees of freedom on this node.")
        end
        for i in 1:ncomps
...
function update!(ch::ConstraintHandler2, time::Real=0.0)
    @assert ch.closed[]
    for dbc in ch.dbcs
        field_idx = find_field(ch.dh, dbc.field_name)
        # Function barrier
...
function WriteVTK.vtk_point_data(vtkfile, ch::ConstraintHandler2)
    unique_fields = []
    for dbc in ch.dbcs
...
function apply!(v::AbstractVector, ch::ConstraintHandler2)
    @assert length(v) == ndofs(ch.dh)
    v[ch.prescribed_dofs] = ch.values # .= ??
    return v
end

function apply_zero!(v::AbstractVector, ch::ConstraintHandler2)
    @assert length(v) == ndofs(ch.dh)
    v[ch.prescribed_dofs] .= 0
    return v
end

function apply!(K::Union{SparseMatrixCSC,Symmetric}, ch::ConstraintHandler2)
    apply!(K, eltype(K)[], ch, true)
end

function apply_zero!(K::Union{SparseMatrixCSC,Symmetric}, f::AbstractVector, ch::ConstraintHandler2)
    apply!(K, f, ch, true)
end

# @enum(ApplyStrategy, APPLY_TRANSPOSE, APPLY_INPLACE)

function apply!(KK::Union{SparseMatrixCSC,Symmetric}, f::AbstractVector, ch::ConstraintHandler2, applyzero::Bool=false;
