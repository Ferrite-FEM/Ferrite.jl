...
n_rows(bsp::BlockSparsityPattern) = bsp.nrows
n_cols(bsp::BlockSparsityPattern) = bsp.ncols
...
eachrow(bsp::BlockSparsityPattern) = (BSPRowIterator(bsp, row) for row in 1:n_rows(bsp))
