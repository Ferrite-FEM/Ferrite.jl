...
### Documentation
 ...
 - All figures now come in light/dark variants, so none of them are unreadable on the dark
   themes. The drawn illustrations in the topic guides are generated by `docs/diagrams.jl`,
   and the remaining ParaView figures (the post-processing and threaded assembly how-tos,
   and the topology optimization result) go through the screenshots pipeline like the rest
   ([#1416], [#1421])
