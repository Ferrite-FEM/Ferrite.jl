...
dh = DofHandler(grid)
add!(dh, :x, Lagrange{1,RefLine,1}()^2)
close!(dh)
...
dh = DofHandler(grid)
add!(dh, :x, Lagrange{1,RefLine,2}()^2)
close!(dh)
...
dh = DofHandler(grid)
add!(dh, :u, Lagrange{1,RefLine,2}()^3)
add!(dh, :θ, Lagrange{1,RefLine,2}()^3)
close!(dh)
...
dh = DofHandler(grid)
add!(dh, :u, Lagrange{2,RefQuadrilateral,1}()^3)
add!(dh, :θ, Lagrange{2,RefQuadrilateral,1}()^3)
close!(dh)
...
@testset "renumber!" begin
    function dhmdhch()
        ...
        # subdomains
        mdh = DofHandler(grid)
        add!(mdh, FieldHandler([Field(:u, Lagrange{2,RefTriangle,1}())], Set(1:getncells(grid)÷2)))
        add!(mdh, FieldHandler([Field(:u, Lagrange{2,RefTriangle,1}())], Set((getncells(grid)÷2+1):getncells(grid))))
        close!(mdh)
...
@testset "dof coupling" begin
    ...
    dh = DofHandler(grid)
    add!(dh, :u, Lagrange{2,RefQuadrilateral,1}()^2)
    add!(dh, :p, Lagrange{2,RefQuadrilateral,1}())
    close!(dh)
    ch = ConstraintHandler(dh)
    close!(ch)
    ...
    dh = DofHandler(grid)
    fh1 = FieldHandler(
        [Field(:u, Lagrange{2,RefQuadrilateral,1}()^2), Field(:p, Lagrange{2,RefQuadrilateral,1}()^2)],
        Set(1)
    )
    add!(dh, fh1)
    fh2 = FieldHandler(
        [Field(:u, Lagrange{2,RefQuadrilateral,1}()^2)],
        Set(2)
    )
    add!(dh, fh2)
    close!(dh)
