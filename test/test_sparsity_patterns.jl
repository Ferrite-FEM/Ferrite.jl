...
@testset "SparsityPattern" begin
    ...
    compare_matrices(
        ...
        let
            A = create_matrix(SparseMatrixCSC{Float32, Int}, dh)
            @test A isa SparseMatrixCSC{Float32, Int}
            A
        end,
        let
            dsp = create_sparsity_pattern(dh)
        ...
        let
            dsp = create_sparsity_pattern(dh)
            A = create_matrix(SparseMatrixCSC{Float32, Int32}, dsp)
            @test A isa SparseMatrixCSC{Float32, Int32}
            A
        end,
        let
            dsp = SparsityPattern(ndofs(dh), ndofs(dh); nnz_per_row = 5)
            create_matrix(create_sparsity_pattern!(dsp, dh))
        end,
    )
    ...
    compare_matrices(
        let
            A = create_matrix(dh, ch)
            @test A isa SparseMatrixCSC{Float64, Int}
            A
        end,
        let
            A = create_matrix(SparseMatrixCSC{Float32, Int}, dh, ch)
            @test A isa SparseMatrixCSC{Float32, Int}
            A
        end,
        let
            dsp = create_sparsity_pattern(dh, ch)
            @test dsp isa SparsityPattern
            A = create_matrix(dsp)
            @test A isa SparseMatrixCSC{Float64, Int}
            A
        ...
        let
            dsp = SparsityPattern(ndofs(dh), ndofs(dh))
            create_matrix(create_sparsity_pattern!(dsp, dh, ch))
        end,
    )
    ...
    compare_matrices(
        let
            A = create_matrix(dh, ch; kwargs...)
            @test A isa SparseMatrixCSC{Float64, Int}
            A
        end,
        let
            A = create_matrix(SparseMatrixCSC{Float32, Int}, dh, ch; kwargs...)
            @test A isa SparseMatrixCSC{Float32, Int}
            A
        end,
        let
            dsp = create_sparsity_pattern(dh, ch; kwargs...)
            @test dsp isa SparsityPattern
            A = create_matrix(dsp)
            @test A isa SparseMatrixCSC{Float64, Int}
            A
        ...
        let
            dsp = SparsityPattern(ndofs(dh), ndofs(dh))
            create_matrix(create_sparsity_pattern!(dsp, dh, ch; kwargs...))
        end,
    )
...
@testset "Sparsity pattern generics" begin
    ...
    close!(dh)
    ch = ConstraintHandler(dh)
    add!(ch, Dirichlet(:p, union(getfaceset.(Ref(grid), ("left", "right", "top", "bottom", "front", "back"),)...), x -> 0))
    add!(ch, PeriodicDirichlet(:u, collect_periodic_faces(grid)))
    close!(ch)
    ...
    for p in ps
        create_sparsity_pattern!(p, dh, ch)
        condense_sparsity_pattern!(p, ch)
    end
    compare_patterns(ps...)
