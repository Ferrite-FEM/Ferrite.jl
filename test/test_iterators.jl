@testset "Iterators" begin
    ...
    @testset "FacetIterator" begin
        @testset "Single domain" begin
            ...
            fi_dh = FacetIterator(dh, topology)
            fi_grid = FacetIterator(grid, topology)
            @test fi_dh.set == fi_grid.set

            # Test for allocations
            # TODO: find a way to avoid resize for single sdh
            # @test (testallocs_iterate(fi_dh)) === nothing
            # Iterators over grid can allocate due to potential resize!
            @test_throws AllocCheckFailure testallocs_iterate(fi_grid)
        end
        ...
        @testset "subdomains" begin
            @testset "same cell type" begin
                ...
                fi_sdh1 = FacetIterator(sdh1, topology)
                fi_sdh2 = FacetIterator(sdh2, topology)
                @test isempty(fi_sdh1.set ∩ fi_sdh2.set)
                @test length(fi_sdh1.set) == 12
                # Test for allocations
                @test (testallocs_iterate(fi_sdh1)) === nothing
                @test (testallocs_iterate(fi_sdh2)) === nothing
            end
            ...
            @testset "mixed cell types" begin
                ...
                fi_quad = FacetIterator(sdh_quad, topology)
                fi_tri = FacetIterator(sdh_tri, topology)
                @test isempty(fi_quad.set ∩ fi_tri.set)
                # Test for allocations
                @test (testallocs_iterate(fi_quad)) === nothing
                @test (testallocs_iterate(fi_tri)) === nothing
            end
        end

        @testset "FacetIndex" begin
            # checkmate, codecov bot!
            @test isequal(FacetIndex(1, 2), (1, 2))
            @test isequal((1, 2), FacetIndex(1, 2))
        end
        ...
        @testset "error paths" begin
            ...
            add!(sdh_tri, :u, ip_tri)
            close!(dh)
            @test_throws ErrorException("The cells in the set (set of FacetIndex) are not all of the same celltype.") FacetIterator(dh, topology)
        end
    end
