@testset "initial_values" begin
    ...
    @testset "Exceptions" begin
        ...
        @test_throws ErrorException apply_analytical!(zeros(ndofs(dh)), dh, :v, x->0.0)
        @test_throws ErrorException apply_analytical!(zeros(ndofs(dh)), dh, :u, x->0.0)     # Should be f(x)::Vec{2}

        mdh = testmdh(2, 1, 1)
        @test_throws ErrorException apply_analytical!(zeros(ndofs(mdh)), mdh, :v, x->0.0)
