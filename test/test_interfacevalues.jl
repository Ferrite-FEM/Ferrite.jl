@testset "InterfaceValues" begin
    ...
    for (cell_shape, scalar_interpol, quad_rule) in (
        ...
        @testset "error paths" begin
            ...
            @test_throws ArgumentError Ferrite.element_to_facet_transformation(Vec{dim, Float64}(ntuple(_ -> 0.0, dim)), Ferrite.getrefshape(cell), 100)
            @test_throws ArgumentError Ferrite.facet_to_element_transformation(Vec{dim - 1, Float64}(ntuple(_ -> 0.0, dim - 1)), Ferrite.getrefshape(cell), 100)
        end
